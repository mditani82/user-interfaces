...
var _RoomAlertModalComponent = class _RoomAlertModalComponent {
  ...
  async save() {
    this.loading = true;
    const metadata = await fu(this._org.organisation.id, "room_alerts").toPromise().catch((e) => {
...
var _RoomModalComponent = class _RoomModalComponent extends AsyncHandler {
  constructor() {
    ...
    this.query_fn = (_) => Nc({ q: _ }).pipe(map((resp) => resp.data));
  ...
  async ngOnInit() {
    const { details } = await fu(this._org.organisation.id, "settings").toPromise();
  ...
  async save() {
    ...
    await lastValueFrom(hu(this._org.organisation.id, {
      name: "settings",
    ...
    await lastValueFrom(data.id ? cc(data.id, data) : ac(data));
...
//# sourceMappingURL=room-manager.module-6UQGKMGM.js.map
